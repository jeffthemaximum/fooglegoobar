...
def mover(char):
    less = ord(char) - 97
    return chr(abs(less - 25) + 97)

def answer(string):
    split = list(string)
    new_string = []
    for char in split:
        is_lower = char.lower() == char and ord(char) > 96 and ord(char) < 123
        if is_lower:
            new_string.append(mover(char))
        else:
...
def solution(s):
    return "".join([chr(abs(ord(char) - 122) + 97) if char.islower() else char for char in list(s)])
